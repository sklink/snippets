...
module.exports = {
  activate () {
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor', {
      'snippets:expand'(event) {
        ...
        if (!snippets.expandSnippetsUnderCursors(editor)) {
  ...
  expandSnippetsUnderCursors (editor) {
    ...
    editor.transact(() => {
      ...
      const group = this.newExpansionGroup(editor)
      ...
      for (const cursor of cursors) {
        ...
        this.insert(snippet, editor, cursor, group)
      }
    })
    return true
  },
  ...
  goToNextTabStop (editor) {
    let group = this.getExpansionGroup(editor)

    while (group) {
      let ref = group.expansions;
      ...
      if (group.expansions.length) {
        return true;
      }

      group = this.nextExpansionGroup(editor);
    }
  ...
  getExpansionGroup (editor) {
    this.editorSnippetExpansions.get(editor)
  },

  nextExpansionGroup (editor) {
    let group = this.editorSnippetExpansions.get(editor)?.parent
    this.editorSnippetExpansions.set(editor, group)
  ...
  newExpansionGroup (editor) {
    let group = {
      parent: this.editorSnippetExpansions.get(editor),
      expansions: []
    };
  ...
  insert (snippet, editor, cursor, group = this.newExpansionGroup(editor)) {
    ...
    return new SnippetExpansion(snippet, editor, cursor, this, group)
  },
